...
test('professional admin console is accessible and session protected', async () => {
  ...
  assert.match(login, /VasiAccountRole\.scoped\('admin'\)/);
  assert.match(login, /storageKey:'vasi-admin-auth'/);
  assert.match(login, /signOut\(\{scope:'local'\}\)/);
  assert.match(app, /adminRole\?\.active\(\)/);
  assert.match(app, /storageKey:'vasi-admin-auth'/);
...
test('admin live GPS keeps one responsive map and refreshes markers in place', async () => {
  ...
  assert.match(app, /if\(changed\)mapTiles\?\.getMaplibreMap\?\.\(\)\?\.resize\(\)/);
  ...
  assert.match(app, /function professionalVectorLayer\(\)/);
  assert.match(app, /https:\/\/tiles\.openfreemap\.org\/styles\/positron/);
  assert.match(app, /mapTiles=professionalVectorLayer\(\)\.addTo\(map\)/);
  assert.match(app, /mapTiles\.getMaplibreMap\(\)/);
