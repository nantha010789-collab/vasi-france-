export default async function handler(req,res){
 ...
 if(req.method==='GET'){
  const r=await fetch(`${url}/rest/v1/drivers?select=id,full_name,phone,online,verified,latitude,longitude,vehicle_make,vehicle_model,vehicle_plate,vehicle_color,rating,created_at&order=created_at.desc&limit=100`,{headers:h});
 ...
 const patch={};if(typeof body.verified==='boolean')patch.verified=body.verified;if(typeof body.online==='boolean')patch.online=body.online;if(!Object.keys(patch).length)return res.status(400).json({error:'No supported change'});
