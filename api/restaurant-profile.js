...
async function config(req){
  ...
  const r=await fetch(`${proto}://${host}/api/public-client-config`,{headers:{'User-Agent':'VASI-profile'}});
  const j=await r.json().catch(()=>({}));
  if(!r.ok||!j.url||!j.key)throw new Error('Restaurant service is not configured');
  return j;
}

async function getUser(url,key,auth){
...
async function db(url,key,path,auth,options={}){
...
export default async function handler(req,res){
  ...
  try{
    const {url,key}=await config(req);
    const auth=req.headers.authorization||'';
    const user=await getUser(url,key,auth);
    if(!user)return res.status(401).json({error:'Login required'});
    const rows=await db(url,key,`restaurants?select=*&owner_id=eq.${encodeURIComponent(user.id)}&order=created_at.asc&limit=1`,auth);
    ...
    const updated=await db(url,key,`restaurants?id=eq.${encodeURIComponent(restaurant.id)}&owner_id=eq.${encodeURIComponent(user.id)}`,auth,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(patch)});
    return res.status(200).json({restaurant:updated?.[0]||{...restaurant,...patch}});
  }catch(e){return res.status(Number(e?.status)||500).json({error:e?.message||'Restaurant profile update failed'});}
}
